...
#if defined(NDEBUG)
#error "New808coin cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 80.8 * 60 * 60;  //80.8 hours  = 3.3 days
int64_t nReserveBalance = 0;

/** Fees smaller than this (in un808) are considered zero fee (for relaying and mining)
...
{
    RenameThread("new808coin-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // N808: We use certain sporks during IBD, so check to see if they are
