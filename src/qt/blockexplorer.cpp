...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::N808, nValue);
...
static std::string TxToRow(const CTransaction& tx, const CScript& Highlight = CScript(), const std::string& Prepend = std::string(), int64_t* pSum = NULL)
{
    ...
    for (unsigned int j = 0; j < tx.vout.size(); j++) {
        ...
        if (tx.IsCoinBase() && Out.scriptPubKey.empty())
            OutAddresses += _("PoS marker");
        else if (tx.IsCoinStake() && Out.scriptPubKey.empty())
...
std::string BlockToString(CBlockIndex* pBlock)
{
    ...
    if (pBlock->nHeight == 0)
        ...
        const CAmount nMoneySupplyPrev =
            pBlock->pprev ? pBlock->pprev->nMoneySupply : 0;

        if (pBlock->nHeight >= 1000001)
            Generated =
                GetBlockValue(pBlock->nHeight, nMoneySupplyPrev);
        else
            Generated =
                GetBlockValue(pBlock->nHeight - 1, nMoneySupplyPrev);
    }
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (new808coin.conf).");
            QMessageBox::warning(this, "New808coin Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
