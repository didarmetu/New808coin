...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    int nRewardHeight = prev->nHeight;

    if (prev->nHeight >= 1000000)
        nRewardHeight = prev->nHeight + 1;

    txNew.vout[0].nValue =
        GetBlockValue(nRewardHeight, prev->nMoneySupply);
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("New808coinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("New808coinMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("New808coinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("new808coin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running New808coinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
